...
└── Packages/
    ...
    └── CacaoTransEngine/          # macOS 専用エンジン
        ...
        ├── AudioCompressor.swift      # 同梱用に AAC モノラル 48kbps へ圧縮
...
for s in CacaoClaudeTrans CacaoTrans cacaoclaudetrans-cli; do
...
2. ⌘O で音声ファイルを開くと「この音声について」シートが出る。背景（目的・話し手の立場・時代や場所）と用語集
   （人名・地名・専門用語、1行1つ）は **「共通」と「この音声だけの追記」の2段**。共通の欄は「文字起こしを開始」で
   初期値として保存されるので、似た音声を続けて処理するときは追記だけ書けばよい。Claude には共通＋追記をつなげて渡し、
   その内容がプロジェクト（`.ccot`）に保存され、「Claude で校正だけやり直す」でも使われる。あとから直すときはサイドバーの
   「この音声について」→「編集…」。初回は認識モデル・話者分離モデルのダウンロードで数分かかる。
...
4. 発話の分割と統合。本文をクリックして分けたい位置にカーソルを置き ⌘↩ で分割、⌥⌘↩ で句点（。？！）ごとに分割、
   ...
   書き出し時に無視される。仕上げに ⇧⌘J（編集メニュー「連続する同じ話者の発話をすべてつなげる」）で、同じ話者が続く発話を
   まとめて 1 つにできる。行を 1 件だけ選ぶ（↑↓ キーで移動しても）とその発話が再生対象になり、F8 でそこから再生できる。
5. ⌘F で検索・置換パネル。「機械 → 機会」のような誤変換を全発話にまとめて置換。正規表現も使える。一致した箇所は本文中で
   黄色く強調される（その行をクリックすると通常の編集表示に戻る）。件数の下の「一致する発話だけ表示」をオンにすると、
   一致した発話だけが一覧に残る。「クリア」で検索語・置換語・絞り込みを空にする。⌘Z で元に戻す。
6. 各行の ▶ でその発話の音声を再生できる。画面下の再生バーで再生／一時停止（F8。キーボードのメディアキーでも可）、前後の発話（F7 / F9、⌘↑ / ⌘↓）、速度、連続再生、
...
7. ⌘E で書き出し（Word / テキスト / Markdown / 字幕 SRT / CSV）。⌘S でプロジェクト（`.ccot`）として保存すれば後で開いて編集を続けられる。Finder でダブルクリックしても開ける
   （以前の `.cacaotrans` も開ける）。ウインドウの赤い ● で閉じるとアプリも終了する（未保存なら保存するか確認）。
8. **音声の同梱**（1.2.0〜）。既定では文字起こしの最後に音声を AAC モノラル 48kbps に圧縮してプロジェクトに同梱する
   （1時間で 20MB ほど）。`.ccot` を 1 つ渡すだけで作業員の Mac で再生できる。設定（⌘,）→「文字起こし」で
   オフにでき、「文字起こし」メニューの「音声を圧縮してプロジェクトに同梱」「同梱した音声を外す」であとから付け外しできる
   （1.1.0 で作った `.ccot` を開いて同梱することもできる）。サイドバーの「音声」欄に同梱の有無とサイズが出る。
   同梱していても `.ccot` は JSON のままなので、旧バージョンの CacaoTrans でも開ける（音声は別途探す扱いになる）。
...
2. **プロジェクトに同梱された音声**（CacaoClaudeTrans 1.2.0 以降の既定）。作業員側の設定は不要。
3. **音声フォルダ**（既定 `書類/CacaoTrans`）の中の同名ファイル（1階層下のサブフォルダも見る）。
   サンドボックスの制約で、最初に一度だけ「音声フォルダを設定…」でフォルダを選んでもらう（以降は記憶する）。
4. それでも見つからなければ再生バーの「音声ファイルを指定…」で手動選択。

運用の推奨: 作業員の Mac では `/Applications/CacaoTrans.app`。管理者は音声を同梱した `.ccot` を 1 つ渡す。
音声を同梱しない場合は `.ccot` と音声ファイルをセットで渡し、音声は `書類/CacaoTrans/` に置いてもらう。
...
./dist/cacaoclaudetrans 会議.m4a --format json --embed-audio --out 会議.ccot   # 音声同梱のプロジェクトを作る

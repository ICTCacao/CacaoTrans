...
管理者から **プロジェクト（.ccot）が 1 つ**届きます。ふつうは**音声も中に入っている**ので、
これだけで再生しながら作業できます。

| ファイル | 例 | 置き場所 |
|---|---|---|
| プロジェクト（音声入り） | `インタビュー（佐藤）.ccot` | どこでも可（書類 › CacaoTrans がおすすめ） |

**音声が別ファイルで届いた場合だけ**（古い形式のプロジェクトなど）:
...
- サブフォルダ（例: 書類 › CacaoTrans › 2026-09）に入れても見つかります。

---

## 3. 音声が別ファイルのときの設定：音声フォルダ

<div class="cols">
<div>

音声が .ccot に入っていれば**この設定は不要**です。
別ファイルで届いたときだけ、**最初に一度**行います。
...
左のサイドバー「音声」欄に **「同梱 …MB」** と出ていれば、音声はプロジェクトの中にあります。
...
- ウインドウ左上の赤い ● で閉じるとアプリも終了します。未保存なら「保存して終了」を選べます
...
| 症状 | 対処 |
|---|---|
| 「音声ファイルが見つからない」 | 音声が同梱されていないプロジェクトです。音声が **書類 › CacaoTrans** にあるか、名前が変わっていないか確認。それでも出るなら再生バーの **「音声ファイルを指定…」** で直接選ぶか、管理者に音声入りの .ccot をもらう |
